...
def main(config):
    ...
    # Initialize the criterions.
    if config.loss1 == "focal":
        if focal_loss_weights is None:
            ...
            sys.exit(1)
        criterion1 = FocalLoss(weight=focal_loss_weights.to(device))
    elif config.loss1 == "ce":
        criterion1 = nn.CrossEntropyLoss()
    else:
        raise ValueError(
            f"Invalid loss function: {config.loss1}. "
            "Supported options are 'focal' and 'ce'."
        )

    if config.loss2 == "smooth_l1":
        criterion2 = nn.SmoothL1Loss()
    elif config.loss2 == "mse":
        criterion2 = nn.MSELoss()
    elif config.loss2 == "mae":
        criterion2 = nn.L1Loss()
    else:
        raise ValueError(
            f"Invalid loss function: {config.loss2}. "
            "Supported options are 'smooth_l1', 'mse', and 'mae'."
        )
        ...
        with experiment.train():
            ...
            for aug in range(num_aug):
                ...
                for i, data in enumerate(dloaders_train, 0):
                    ...
                    _, inputs, diag_labels, agreement_labels = data

                    # Forward pass.
                    cls_pred, reg_pred = model(inputs)

                    # Compute the loss.
                    loss1 = criterion1(cls_pred, diag_labels)
                    loss2 = criterion2(
                        reg_pred, agreement_labels.unsqueeze(1).float()
                    )
                    loss = (
                        config.cls_loss_weight * loss1
                        + (1 - config.cls_loss_weight) * loss2
                    )
        ...
        with experiment.validate():
            with torch.no_grad():
                ...
                for i, data in enumerate(dloaders_val, 0):
                    _, inputs_eval, diag_labels_eval, agreement_labels_eval = (
                        data
                    )

                    # Forward pass.
                    cls_pred_eval, reg_pred_eval = model(inputs_eval)
                    pred_eval_labels = torch.argmax(cls_pred_eval, dim=1)

                    all_val_GTs = torch.cat((all_val_GTs, diag_labels_eval), 0)
                    all_val_preds = torch.cat(
                        (all_val_preds, pred_eval_labels), 0
                    )
                    all_val_logits = torch.cat(
                        (all_val_logits, cls_pred_eval), 0
                    )
