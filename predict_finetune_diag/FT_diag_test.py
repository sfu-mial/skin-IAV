...
def main(config):
    ...
    # Data loaders (only need test loader, but reuse util).
    ...
    if dloader_payload is None:
        logger.error(
            ...
            "implemented for certain datasets (e.g., 'IAMApp')."
        )
        sys.exit(1)
    ...
    if not ckpt_path.exists():
        logger.error(f"Best/last checkpoint not found at {ckpt_path}")
        sys.exit(1)
        ...
        with torch.no_grad():
            ...
            with open(csv_path, "wt", newline="") as f:
                ...
                for i, data in enumerate(dloaders_test, 0):
                    ...
                    all_test_GTs = torch.cat((all_test_GTs, labels_test), 0)
                    all_test_preds = torch.cat(
                        (all_test_preds, pred_test_labels), 0
                    )
                    all_test_logits = torch.cat(
                        (all_test_logits, pred_test), 0
                    )

                    # Write predictions to CSV.
                    probs_test = F.softmax(pred_test, dim=1)
                    responses = [
                        [
                            f"{names_test[idx]}",
